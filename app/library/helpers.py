...
def setdimensions(winWidth, imgWidth, imgHeight):
    ...
    max_imgWidth = new_imgWidth
    # max_imgWidth = settings.max_imgWidth
    max_imgHeight = int(imgHeight * ratio * 0.8)
...
def create_workspace(path_root):
    """
    Return workspace path
    """
    # base directory
    # path_root = 'Upload/image'
    work_dir = Path(path_root)
    # print(work_dir)
...
def thumb(myfile, winWidth, imgWidth, imgHeight):
    ...
    # print(imgtype)
    path = filepath + "_thumbnail"+ext
    
    im.save(path, imgtype)
    return path
    
def img_processing(myfile):
    # size = setdimensions(winWidth, imgWidth, imgHeight)
    # size = settings.thumb_width, settings.thumb_height
    filepath, ext = os.path.splitext(myfile)
    # print(ext)
    ...
    imgtype = "PNG" if ext == ".png" else "JPEG"
    # print(imgtype)
    save_path = filepath + "_gray"+ext
    
    im.save(save_path, imgtype)
    return save_path
